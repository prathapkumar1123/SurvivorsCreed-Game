...
public class ExplosiveBarrelScript : MonoBehaviour
{
    ...
    private IEnumerator Explode()
    {
        ...
        foreach (Collider hit in colliders)
        {
            ...
            //If the gas tank explosion hits zombies with the tag "Zombie"
            if (hit.transform.tag == "Zombie")
            {
                //Toggle the isHitByExplosion bool on the Zombie object
                hit.transform.gameObject.GetComponent<AIZombieBehaviorScript>().isHitByExplosion = true;
            }
        }
